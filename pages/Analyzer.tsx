...
const Analyzer: React.FC = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      {/* Header */}
      <div className="mb-8 border-b border-trust-800 pb-4">
        <h1 className="text-3xl font-bold text-white mb-2">Forensic Audio Analyzer</h1>
        <p className="text-gray-400 font-mono text-sm">MODULE: DEEPFAKE_DETECTION_V2</p>
      </div>
      ...
      <div className="grid lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-6">
            ...
            {/* Upload Area */}
            <div className="bg-trust-800/30 rounded-xl border border-trust-700 p-6">
                <div className="flex items-center justify-between mb-4">
                     <h2 className="text-lg font-bold text-white">Source Input</h2>
                     {file && <span className="text-xs font-mono bg-trust-900 text-trust-400 px-2 py-1 rounded border border-trust-600">READY</span>}
                </div>
                
                <label className="flex flex-col items-center justify-center w-full h-32 border-2 border-trust-600 border-dashed rounded-lg cursor-pointer hover:bg-trust-800/50 transition-colors">
                    <div className="flex flex-col items-center justify-center pt-5 pb-6">
                        ...
                        <p className="mb-2 text-sm text-gray-400"><span className="font-semibold text-trust-400">Click to upload</span> or drag and drop</p>
            ...
            {/* Visualizer */}
            <div className="bg-trust-800/30 rounded-xl border border-trust-700 p-6">
                 <h2 className="text-lg font-bold text-white mb-4">Spectral Visualization</h2>
        ...
        <div className="lg:col-span-1 space-y-6">
            <div className="bg-trust-800/30 rounded-xl border border-trust-700 overflow-hidden h-full flex flex-col">
                <div className="flex border-b border-trust-700 bg-trust-900/50">
                    <button onClick={() => setActiveTab('metadata')} className={`flex-1 py-3 text-xs font-bold uppercase tracking-wider ${activeTab === 'metadata' ? 'text-trust-500 border-b-2 border-trust-500 bg-trust-800/50' : 'text-gray-500 hover:text-gray-300'}`}>Metadata</button>
                    <button onClick={() => setActiveTab('forensics')} className={`flex-1 py-3 text-xs font-bold uppercase tracking-wider ${activeTab === 'forensics' ? 'text-trust-500 border-b-2 border-trust-500 bg-trust-800/50' : 'text-gray-500 hover:text-gray-300'}`}>Forensics</button>
                    <button onClick={() => setActiveTab('platform')} className={`flex-1 py-3 text-xs font-bold uppercase tracking-wider ${activeTab === 'platform' ? 'text-trust-500 border-b-2 border-trust-500 bg-trust-800/50' : 'text-gray-500 hover:text-gray-300'}`}>Source</button>
                    <button onClick={() => setActiveTab('ip')} className={`flex-1 py-3 text-xs font-bold uppercase tracking-wider ${activeTab === 'ip' ? 'text-trust-500 border-b-2 border-trust-500 bg-trust-800/50' : 'text-gray-500 hover:text-gray-300'}`}>IP Trace</button>
                </div>
                            ...
                            {activeTab === 'metadata' && (
                                <ul className="space-y-4 text-sm">
                                    <li className="flex justify-between border-b border-trust-800 pb-2">
                                        ...
                                        <span className="text-white text-right truncate max-w-[150px]">{metadata.fileName}</span>
                                    </li>
                                    <li className="flex justify-between border-b border-trust-800 pb-2">
